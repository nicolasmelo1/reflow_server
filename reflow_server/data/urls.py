...
urlpatterns = [
    re_path(r'(?P<company_id>(\w+(\.)?(-)?(_)?)+)/(?P<form>\w+)/', include([
        ...
        register_can_be_public_url(register_attachment_url(re_path(r'^$', validate_billing(FormularyDataView.as_view()), name='formulary_formulary_data_view'))),
        register_attachment_url(re_path(r'^(?P<dynamic_form_id>\d+)/$', validate_billing(FormularyDataEditView.as_view()), name='formulary_formulary_data_edit_view')),
