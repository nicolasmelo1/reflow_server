...
urlpatterns = [
    re_path(r'^(?P<company_id>(\w+(\.)?(-)?(_)?)+)/', include([
        ...
        re_path(r'^(?P<form>\w+)/', include([
            ...
            re_path(r'^(?P<field_id>\d+)/user/options/$',permission_required(UserFieldTypeOptionsView.as_view()), name='formulary_get_user_field_type_options'),
            re_path(r'^(?P<field_id>\d+)/form/options/$',permission_required(FormFieldTypeOptionsView.as_view()), name='formulary_get_form_field_type_options'),
        ]))
    ]))
]